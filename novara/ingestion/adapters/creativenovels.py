"""CreativeNovels source adapter (creativenovels.com).

CreativeNovels hosts both original English works and translations.
Chapter list is loaded via AJAX POST to wp-admin/admin-ajax.php.

Characteristics:
- Good cover and author metadata
- AJAX-based chapter list (action=crn_chapter_list)
- Content in article .entry-content
"""

from __future__ import annotations

import re

from bs4 import BeautifulSoup, Tag

from novara.ingestion.adapter_utils import (
    absolute_url,
    find_cover,
    inner_text,
    normalise_status,
    parse_chapter_number,
    text,
)
from novara.ingestion.base_adapter import (
    BaseAdapter,
    RawChapterContent,
    RawChapterListing,
    RawMetadata,
)
from novara.ingestion.http_client import fetch_with_retry, rate_limited_client
from novara.ingestion.registry import register

_POST_ID_RE = re.compile(r'"post_id"\s*:\s*"?(\d+)"?')
_SLUG_RE = re.compile(r"creativenovels\.com/novel/([^/?#]+)")


@register
class CreativeNovelsAdapter(BaseAdapter):
    SITE_KEY = "creativenovels"
    SITE_NAME = "Creative Novels"
    BASE_URL = "https://creativenovels.com"
    REQUESTS_PER_SECOND = 0.5

    def extract_source_id(self, source_url: str) -> str:
        m = _SLUG_RE.search(source_url)
        return m.group(1) if m else source_url

    async def scrape_title(self, source_url: str) -> RawMetadata:
        async with rate_limited_client(self.SITE_KEY, self.REQUESTS_PER_SECOND) as client:
            resp = await fetch_with_retry(client, source_url)

        soup = BeautifulSoup(resp.text, "lxml")

        # Title from head title tag (strip trailing site name)
        raw_title = text(soup.select_one("head title")) or ""
        title = raw_title.split("–")[0].split("|")[0].strip() or None

        cover = find_cover(soup, "img.book_cover", ".book-cover img")
        author = text(soup.select_one(".x-bar-content .x-text.bK_C, .author a"))
        synopsis = inner_text(soup.select_one(".description, .summary, .novel-description"))
        status_el = soup.select_one(".status, [class*='status']")
        status = normalise_status(text(status_el))
        genres = [text(a) for a in soup.select("a[href*='/genre/']") if text(a)]

        return RawMetadata(
            title=title,
            synopsis=synopsis,
            author=author,
            status=status,
            language="en",
            genres=genres or [],
            cover_url=cover,
        )

    async def scrape_chapter_listing(self, source_url: str) -> list[RawChapterListing]:
        """Fetch chapter list via AJAX (crn_chapter_list action)."""
        async with rate_limited_client(self.SITE_KEY, self.REQUESTS_PER_SECOND) as client:
            # First load the novel page to get the post ID
            resp = await fetch_with_retry(client, source_url)
            post_id = _extract_post_id(resp.text)

            chapters: list[RawChapterListing] = []

            if post_id:
                ajax_url = f"{self.BASE_URL}/wp-admin/admin-ajax.php"
                ajax_resp = await client.post(ajax_url, data={
                    "action": "crn_chapter_list",
                    "view_id": post_id,
                })
                if ajax_resp.status_code == 200:
                    soup = BeautifulSoup(ajax_resp.text, "lxml")
                    for idx, link in enumerate(soup.select("a"), start=1):
                        if not isinstance(link, Tag):
                            continue
                        href = str(link.get("href", ""))
                        if not href:
                            continue
                        chapter_url = absolute_url(self.BASE_URL, href)
                        chapter_title = text(link)
                        chapter_num = parse_chapter_number(chapter_title, href) or float(idx)
                        chapter_id = href.rstrip("/").split("/")[-1] or str(idx)
                        chapters.append(RawChapterListing(
                            source_chapter_id=chapter_id,
                            source_url=chapter_url,
                            chapter_number=chapter_num,
                            title=chapter_title,
                        ))
                    return chapters

            # Fallback: parse chapter list from the novel page directly
            soup = BeautifulSoup(resp.text, "lxml")
            for idx, link in enumerate(soup.select(".chapter-listing a, .chapter-list a"), start=1):
                if not isinstance(link, Tag):
                    continue
                href = str(link.get("href", ""))
                if not href:
                    continue
                chapter_url = absolute_url(self.BASE_URL, href)
                chapter_title = text(link)
                chapter_num = parse_chapter_number(chapter_title, href) or float(idx)
                chapter_id = href.rstrip("/").split("/")[-1] or str(idx)
                chapters.append(RawChapterListing(
                    source_chapter_id=chapter_id,
                    source_url=chapter_url,
                    chapter_number=chapter_num,
                    title=chapter_title,
                ))

        return chapters

    async def scrape_chapter(self, chapter_url: str) -> RawChapterContent:
        async with rate_limited_client(self.SITE_KEY, self.REQUESTS_PER_SECOND) as client:
            resp = await fetch_with_retry(client, chapter_url)

        soup = BeautifulSoup(resp.text, "lxml")
        title = text(soup.select_one("h1, .chapter-title"))
        content_el = soup.select_one("article .entry-content, .chapter-content")
        raw_html = str(content_el) if content_el else ""
        chapter_id = chapter_url.rstrip("/").split("/")[-1]
        chapter_num = parse_chapter_number(title, chapter_url) or 0.0

        return RawChapterContent(
            source_chapter_id=chapter_id,
            source_url=chapter_url,
            chapter_number=chapter_num,
            title=title,
            raw_content=raw_html,
            content_format="html",
        )


def _extract_post_id(html: str) -> str | None:
    m = _POST_ID_RE.search(html)
    return m.group(1) if m else None
